...
import UseAddAlladmins from "../../../../hooks/HAdmins/Alladmins/UseAddAlladmin";
...
const AddAlladmin = ({ isOpen, closeModal, title }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const { addAlladmins, loading } = UseAddAlladmins();
  const storageKey = "logged";
  const userDataString = localStorage.getItem(storageKey);
  const userData = userDataString ? JSON.parse(userDataString) : null;

  const onSubmit = async (data) => {
    const result = await addAlladmins(data, userData);
    if (result.success) {
      closeModal();
