...
import UseaddRoomadmin  from "../../../../hooks/HAdmins/Room_admins/UseAdd_roomadmin";
...
const AddRoomadmin = ({ isOpen, closeModal, title }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const { addRoomadmin, loading } = UseaddRoomadmin();
  const storageKey = "logged";
  const userDataString = localStorage.getItem(storageKey);
  const userData = userDataString ? JSON.parse(userDataString) : null;

  const onSubmit = async (data) => {
    const result = await addRoomadmin(data, userData);
    if (result.success) {
      closeModal();
